...
layui.use('table', function(){
    ...
    table.render({
        elem: '#test'
        ,url:'../SuggestServlet?opt=queryAll'
        ,toolbar: '#toolbarDemo'
        ,cellMinWidth: 40
        ,title: '提议表'
        ,cols: [[
            ...
            layer.open({
                ...
                ,success: function(layero){
                    ...
                    btn.find('.layui-layer-btn').attr({
                        href: '../SuggestServlet?opt=queryAll'
                        ,target: '_blank'
    ...
    table.on('tool(test)', function(obj){
        ...
        switch(obj.event){
            case 'del':
                ...
                break;
            case 'edit':
               updateSuggest(data.employee_id);
                break;
            case 'det':
              //  detGroup(data.user_id);
                break;
            case 'agree':
            	agree(data.name,data.work_exper,data.employee_id,data.reserveposition);
            	break;
        };
    });
...
function agree(name,work_exper,employee_id,reserveposition){
	//alert(name);
	window.location.href="../change/AddChange.jsp?name="+name+"&work_exper="+work_exper+"&employee_id="+employee_id+"&reserveposition="+reserveposition+"";
...
function  delSuggest(id){
    ...
    $.ajax({
        type:'POST',
        url: '../SuggestServlet?opt=del&employee_id='+id,
...
function  updateSuggest(id) {
    layer.open({
        ...
        ,btnAlign: 'c'
        ,moveType: 1 //拖拽模式，0或者1
        ,content: '../SuggestServlet?opt=findById2&employee_id='+id
        ,end:function () {
            location.reload();
        }
        ,success: function(layero){
...
<script type="text/html" id="barDemo">
    ...
    <a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="agree">同意</a>
</script>
